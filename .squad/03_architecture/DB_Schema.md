...
| 키 패턴 | 타입 | TTL | 용도 |
|---------|------|-----|------|
| `feed:news:p{page}:l{limit}` | String (JSON) | 300초 (5분) | 뉴스 탭 피드 API 응답 캐시 (v1.1: 오프셋 기반) |
| `feed:vc_blog:p{page}:l{limit}` | String (JSON) | 300초 (5분) | VC 블로그 탭 피드 API 응답 캐시 (v1.1: 오프셋 기반) |
