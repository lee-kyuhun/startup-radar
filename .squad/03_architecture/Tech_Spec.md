# Tech Spec: Startup Radar — MVP v1.0

> 작성: Tech Lead 에이전트 | 날짜: 2026-02-21 | 상태: 승인 대기
> 기준 문서: `.squad/01_planning/PRD.md`, `.squad/02_design/UI_Specs.md`
...
| 프론트 상태 관리 | Tanstack Query | v5 | 서버 상태 캐싱, 무한 스크롤, 로딩/에러 상태 통합 |
...
| 엔드포인트 | 메서드 | 용도 | 우선순위 |
|-----------|--------|------|---------|
| `/api/v1/feed/` | GET | 탭별 피드 목록 (커서 페이지네이션) | P0 |
...
**피드 커서 페이지네이션:** `published_at:id` Base64 인코딩
...
*Tech Lead 에이전트 작성 | 2026-02-21 | 다음 단계: 사용자 승인 → hired_agents.json gates.tech_spec_approved = true → 빌드 게이트 진입*
